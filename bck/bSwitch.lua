...
function OnJoin(PlayerIndex)
    local thisPlayer = get_player(PlayerIndex)
    local playerTeamIndex = read_byte(thisPlayer + 0x20);
    if playerTeamIndex == 0 then --if player is on team 0
        say(PlayerIndex, "You are on team 0")
        CHOSEN_BIPEDS[get_var(PlayerIndex, "$hash")] = 0
    else
        say(PlayerIndex, "You are on team 1")
        CHOSEN_BIPEDS[get_var(PlayerIndex, "$hash")] = 1
    end
end
