...
carEvent = false
	...
	MAXKARMA = 10
	maxAmmoPrice = 7500
	loadoutChangePrice = 5000
	ClaimedRewards = {}
	LOCATIONS = {
		["dealership_1"] = "the downtown dealership.",
		["dealership_2"] = "the anarchist dealership.",
		["garage_5"] = "the downtown garage.",
		["apartments"] = "the apartments.",
		["gunstore_3"] = "the downtown ammunation.",
		["garage_1"] = "a garage.",
		["garage_2"] = "a garage.",
		["garage_3"] = "a garage.",
		["garage_4"] = "a garage.",
		["gunstore_1"] = "an ammunation.",
		["gunstore_2"] = "an ammunation.",
	...
	WEAPONS = {
		...
		["ak47"] = "gta_halo\\weapons\\ak47\\ak47",
	...
	VEHICLES = {
		...
		["countach"] = "twisted_metal\\vehicles\\_ff\\countach",
	...
	VEHICLEPRICES = { --prices are defined by taking the counterpart MSRP's of the vehicles, and dividing it by two.
		...
		["challenger"] = 80000,
		["compact"] = 900000,
		["charger"] = 75000, --does not use price scale
		["rx7"] = 625000, --does not use price scale
		["zonda"] = 175000,
		["bfinjection"] = 40000,
		["clover"] = 30000,
		["glendale"] = 7000,
	...
	if desync() == false then
		if player_present(PlayerIndex) then
			...
			elseif commandargs[1] == "redeem" then
				...
				-- 				ActivePlayers[PlayerIndex]:payBank(freeMoney)
			...
			elseif commandargs[1] == "check" then
				...
				elseif commandargs[1] == "stats" then
					rprint(PlayerIndex, "Player Statistics for: "..localPlayer:getName())
					rprint(PlayerIndex, "You are a "..localPlayer:getProfession())
					rprint(PlayerIndex, "You have "..localPlayer:getKarma().." points.")
				...
				return false
			elseif commandargs[1] == "money" then --/checkstatus -> debug function
				local aTable = ActivePlayers[PlayerIndex]
				rprint(PlayerIndex, "MONEY: "..niceMoneyDisplay(localPlayer:getBucks()))
				return false
			elseif commandargs[1] == "loadout" then
				if playerIsInArea(PlayerIndex, "gunstore") then
